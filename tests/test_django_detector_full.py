...
def test_django_global_patterns(django_detector, tmp_path):
    ...
    settings_py.write_text("MIDDLEWARE = ['django.middleware.security.SecurityMiddleware', 'myapp.middleware.SimpleMiddleware']")
    ...
    def find_nodes_impl(kind=None, name_pattern=None, limit=10, **kwargs):
        ...
        if name_pattern == "user_list":
            return [Node(id="view1", kind=NodeKind.FUNCTION, name="user_list", qualified_name="user_list", file_path="views.py", start_line=1, end_line=1, language="python")]
        return []
