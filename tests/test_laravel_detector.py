...
def test_detect_model(detector):
    ...
    extends_edge = Edge(source_id="cls1", target_id="model_base", kind=EdgeKind.EXTENDS, metadata={"target_name": "Model"})
...
def test_detect_other_classes(detector):
    nodes = [
        Node(id="c1", kind=NodeKind.CLASS, name="UserEvent", qualified_name="App\\Events\\UserEvent", file_path="app/Events/UserEvent.php", start_line=1, end_line=10, language="php"),
        Node(id="c2", kind=NodeKind.CLASS, name="UserListener", qualified_name="App\\Listeners\\UserListener", file_path="app/Listeners/UserListener.php", start_line=1, end_line=10, language="php"),
        Node(id="c3", kind=NodeKind.CLASS, name="AuthMiddleware", qualified_name="App\\Http\\Middleware\\AuthMiddleware", file_path="app/Http/Middleware/AuthMiddleware.php", start_line=1, end_line=10, language="php"),
        Node(id="c4", kind=NodeKind.CLASS, name="AppServiceProvider", qualified_name="App\\Providers\\AppServiceProvider", file_path="app/Providers/AppServiceProvider.php", start_line=1, end_line=10, language="php"),
        Node(id="c5", kind=NodeKind.CLASS, name="UserController", qualified_name="App\\Http\\Controllers\\UserController", file_path="app/Http/Controllers/UserController.php", start_line=1, end_line=10, language="php"),
    ]
...
def test_extract_routes(detector, tmp_path):
    ...
    store.find_nodes.side_effect = lambda kind, name_pattern=None, limit=None: [
        Node(id="ctrl1", kind=NodeKind.CLASS, name="UserController", qualified_name="UserController", file_path="UserController.php", start_line=1, end_line=10, language="php"),
        Node(id="ctrl2", kind=NodeKind.CLASS, name="PostController", qualified_name="PostController", file_path="PostController.php", start_line=1, end_line=10, language="php"),
        Node(id="ctrl3", kind=NodeKind.CLASS, name="ApiController", qualified_name="ApiController", file_path="ApiController.php", start_line=1, end_line=10, language="php")
    ]
    ...
    get_route = next(n for n in pattern.nodes if n.metadata.get("http_method") == "GET" and n.metadata.get("url_pattern") == "/users")
    ...
    api_route = next(n for n in pattern.nodes if n.metadata.get("http_method") == "GET" and n.metadata.get("url_pattern") == "/api/status")
...
def test_extract_event_listener_mappings(detector, tmp_path):
    ...
    def mock_find_nodes(kind, name_pattern, limit):
        if name_pattern == "Registered" and kind == NodeKind.EVENT:
            return [Node(id="evt1", kind=NodeKind.EVENT, name="Registered", qualified_name="Registered", file_path="test.php", start_line=1, end_line=10, language="php")]
        if name_pattern == "SendEmailVerificationNotification" and kind == NodeKind.LISTENER:
            return [Node(id="lst1", kind=NodeKind.LISTENER, name="SendEmailVerificationNotification", qualified_name="SendEmailVerificationNotification", file_path="test.php", start_line=1, end_line=10, language="php")]
        return []
...
def test_detect_global_patterns(detector, tmp_path):
    ...
    store = MagicMock()
    store.find_nodes.side_effect = lambda kind, name_pattern=None, limit=None: [
        Node(id="f1", kind=NodeKind.FILE, name="artisan", qualified_name="artisan", file_path=str(tmp_path / "artisan"), start_line=1, end_line=10, language="php")
    ] if kind == NodeKind.FILE else [
        Node(id=f"n_{name_pattern}", kind=kind, name=name_pattern or "test", qualified_name=name_pattern or "test", file_path="test.php", start_line=1, end_line=10, language="php")
    ]
