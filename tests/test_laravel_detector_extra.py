...
def test_extract_event_listener_mappings_missing_nodes(detector, tmp_path):
    ...
    def mock_find_nodes(kind, name_pattern, limit):
        if name_pattern == "FoundEvent":
            return [Node(id="evt1", kind=NodeKind.EVENT, name="FoundEvent", qualified_name="FoundEvent", file_path="test.php", start_line=1, end_line=10, language="php")]
        return []
...
def test_resolve_controller_branches(detector):
    ...
    store.find_nodes.return_value = [
        Node(id="m1", kind=NodeKind.METHOD, name="index", qualified_name="index", file_path="UserController.php", start_line=1, end_line=10, language="php")
    ]
    ...
    # Test just class
    store.find_nodes.return_value = [
        Node(id="c1", kind=NodeKind.CLASS, name="UserController", qualified_name="UserController", file_path="UserController.php", start_line=1, end_line=10, language="php")
    ]
